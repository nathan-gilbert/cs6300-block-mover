 ...
 * I decided to split up the algorithm into separate functions to promote ease
 * of reading, I felt that having one ``generic'' search function, parameterized
 * to morph into BFS/DFS/A* upon the values it was feed to be somewhat hard to
 * read and follow.
 *
 */
...
bool check_goal(Node poss) {
  ...
  for (int i = 0; i < (int)relevant_tables.size(); i++) {
    if (goal[relevant_tables[i]].size() !=
        poss.getState()[relevant_tables[i]].size())
      return false;

    for (int j = 0; j < (int)goal[relevant_tables[i]].size(); j++) {
      if (goal[relevant_tables[i]][j] ==
          poss.getState()[relevant_tables[i]][j]) {
        true_count_table++;
      }
    }
  ...
  for (int i = 0; i < (int)relevant_tables.size(); i++) {
    ...
    if (node_table_size == goal_table_size) {
      ...
      for (int j = 0; j < node_table_size; j++) {
        if (goal[relevant_tables[i]][j] !=
            n.getState()[relevant_tables[i]][j]) {
          h += (goal_table_size - j);
        }
      }
 ...
 * This is equivalent to finding an optimal solution to the relaxed problem
 * which allows one to move blocks anywhere.
 */
void getAltF(Node &n) {
  vector<int> relevant_tables;
  int h = 0;
