...
void Node::addPath(string s) { backpath.append(s + "\n"); }
...
void Node::prettyPrint(int tables, bool final, bool heur) {
  ...
  if (!final)
    if (!heur)
      ...
      cout << "Expanding node: (g=" << cost << ", h=" << heuristic
           << ", f=" << estimate << "):\n"
           << endl;
  else
    cout << "Solution node: \n" << endl;
...
void Node::setDepth(int d) { depth = d; }

void Node::setH(int h) { heuristic = h; }

int Node::getH() { return heuristic; }

void Node::updateF() { estimate = cost + heuristic; }

int Node::getF() const { return estimate; }
...
/* Less than defined in terms of greater-than. A quirck of Priority Queues in
 * C++ */
bool Node::operator<(const Node rhs) const { return (estimate > rhs.getF()); }
