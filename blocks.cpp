 ...
 * error checking their syntax. A few helper functions that are used throughout the
 * project, such as ``min'' are also defined here.
 */
#include "blocks.h"
...
/*
 * This reads in the input file, tests for correct syntax, returns true if everything goes well,
 * else false.
 *
 * Also creates the initial and goal states, but does not create their nodes. This is done in moveblocks.cpp.
 */
bool readInit(const char *inFile, int &tables,int &blocks, vector<vector<char> > &is, vector<vector<char> > &gs)
{
    ifstream in;
    string line, buf;
    int table = -1;
    ...
    while(!in.eof())
    {
        ...
        table = atoi(line.substr(line.find_last_of("table")+1, line.find(":")+1).c_str());
...
/*
 * This function handles printing the inital and goal states after they have been
 * correctly parsed and read from file. This function is only called once per run.
 */
