...
struct poptOption OptionsTable[] = {
    {"bfs", 'b', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &bfs, 0,
     "Use breadth-first-search.", NULL},
    {"dfs", 'd', POPT_ARG_INT | POPT_ARGFLAG_ONEDASH, &depth_value, 0,
     "Use depth-first-search, the integer depth limit following the -dfs flag, "
     "indicates the maximum number of moves to be explored along any path from "
     "the start node.",
     NULL},
    {"astar", 'a', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &astar, 0,
     "Use the A* search algorithm.", NULL},
    {"alth", 'h', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &alth, 0,
     "Use an alternate heuristic function, cannot be used with bfs or dfs.",
     NULL},
    {"trace", 't', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &trace, 0,
     "List each node in the search space as it is expanded, along with "
     "relevant information about path cost and heuristic function.",
     NULL},
    {"stats", 's', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &stats, 0,
     "Output information about the efficency of the search.", NULL},
    {"debug", 'g', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &debug, 0,
     "Print out the path from solution to start node and all relevant "
...
int main(int argc, char *argv[]) {
  ...
  if (!readInit(args[argpt++], tables, blocks, initial, goal)) {
    ...
    cout << "There could be several reasons for this, make sure file syntax is "
            "correct and that the file actually exists. "
         << endl;
